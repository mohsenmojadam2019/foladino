...
<main class="invoice-page"><div class="shell"><article class="invoice-card">
  <header><div><h1>{{ $order->invoice_type === 'official' ? 'فاکتور رسمی' : 'پیش‌فاکتور فروش' }} فولادینو</h1><p>قیمت شفاف · تأمین مطمئن · تحویل سراسر کشور</p></div><strong>{{ $order->order_number ?: '#'.fa_digits($order->id) }}</strong></header>
  <div class="invoice-meta"><span>خریدار: <b>{{ $order->customer_name }}</b></span><span>موبایل: <b>{{ $order->mobile }}</b></span><span>شهر: <b>{{ $order->city ?: '—' }}</b></span><span>آدرس: <b>{{ $order->delivery_address ?: '—' }}</b></span></div>
  <table><thead><tr><th>محصول</th><th>مقدار</th><th>قیمت واحد</th><th>مبلغ کل</th></tr></thead><tbody>@foreach($order->items as $item)<tr><td>{{ $item->product_name }}</td><td>{{ fa_digits($item->quantity_tons) }} تن</td><td>{{ money_fa($item->unit_price_toman) }}</td><td>{{ money_fa($item->line_total_toman) }}</td></tr>@endforeach<tr><td colspan="3">هزینه حمل</td><td>{{ money_fa($order->shipping_toman) }}</td></tr></tbody></table>
  <footer><span>وضعیت: پرداخت‌شده</span><strong>مبلغ نهایی: {{ money_fa($order->total_toman) }} تومان</strong></footer>
  <a href="{{ route('orders.invoice.pdf',$order->public_token) }}">دانلود PDF</a><button onclick="window.print()">چاپ</button>
</article></div></main>
